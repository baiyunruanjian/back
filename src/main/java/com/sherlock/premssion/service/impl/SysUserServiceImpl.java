...
//        sysUserVo.setSysRoles(sysRoles);
     ...
     */
    @Override
    public R updateUser(SysUser sysUserFrom) {
        ...
        SysUser sysUser = sysUserRepository.findOne(sysUserFrom.getId());
        if (sysUserFrom.getPassword()!=null){
            /*初始化密码与盐并保存*/
            String salt = ShiroUtil.getSalt();
            String md5Password = ShiroUtil.MD5(sysUserFrom.getPassword(),salt);
            sysUser.setPassword(md5Password);
            sysUser.setSalt(salt);
        }
        if (sysUserFrom.getAccount()!=null){
            sysUser.setAccount(sysUserFrom.getAccount());
        }
        if (sysUserFrom.getContacts()!=null){
            sysUser.setContacts(sysUserFrom.getContacts());
        }
        if (sysUserFrom.getPhone()!=null){
            sysUser.setPhone(sysUserFrom.getPhone());
        }
        if (sysUserFrom.getZhuYing()!=null){
            sysUser.setZhuYing(sysUserFrom.getZhuYing());
        }
        if (sysUserFrom.getWangZhi()!=null){
            sysUser.setWangZhi(sysUserFrom.getWangZhi());
        }
        if (sysUserFrom.getBeiZhu()!=null){
            sysUser.setBeiZhu(sysUserFrom.getBeiZhu());
        }
        if (sysUserFrom.getWeiXin()!=null){
            sysUser.setWeiXin(sysUserFrom.getWeiXin());
        }
        if (sysUserFrom.getAddress()!=null){
            sysUser.setAddress(sysUserFrom.getAddress());
        }
